...
import { getPatientNotifications } from '../../utils/database';
  ...
  useEffect(() => {
    if (isFocused && user) {
      const fetchUnreadCount = async () => {
        try {
          const mobile = user?.contactNumber || user?.mobile || 'Patient';
          const notifications = await getPatientNotifications(mobile);
